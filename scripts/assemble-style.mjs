...
.or-team__fade-wrap{position:relative;min-height:min(200px,28vh)}
...
.or-team__controls{display:flex;align-items:center;justify-content:center;gap:12px;margin-top:auto;padding-top:8px;width:100%;min-width:0}
.or-team__strip{display:flex;gap:10px;justify-content:safe center;flex:0 1 auto;min-width:0;max-width:calc(100% - 104px);width:max-content;overflow-x:auto;overflow-y:hidden;-webkit-overflow-scrolling:touch;overscroll-behavior-x:contain;scrollbar-width:thin}
.or-team__face{flex:0 0 auto;width:48px;height:48px;padding:0;border-radius:12px;border:2px solid rgba(255,255,255,.1);background:rgba(0,0,0,.45);cursor:pointer;overflow:hidden;transition:border-color .25s var(--ease),transform .22s var(--ease),box-shadow .25s var(--ease)}
.or-team__face:hover{border-color:rgba(249,115,22,.4);transform:translateY(-2px)}
.or-team__face.is-active{border-color:rgba(251,146,60,.88);box-shadow:0 0 0 1px rgba(249,115,22,.35),0 10px 28px rgba(234,88,12,.18)}
.or-team__face img{width:100%;height:100%;object-fit:cover;display:block}
...
@media (max-width:900px){.or-team__grid{grid-template-columns:1fr;min-height:0}.or-team__left{border-right:0;border-bottom:1px solid rgba(255,255,255,.08)}.or-team__right{order:-1;min-height:260px}.or-team__strip{max-width:calc(100% - 96px)}}
`;
const appWikiReveal = `
