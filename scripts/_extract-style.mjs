...
for (const f of files) {
  ...
  while ((i = txt.indexOf(needle, i)) !== -1) {
    ...
    if (cIdx === -1 || cIdx > i + 400) { i += 1; continue; }
    ...
    while (j < txt.length) {
      const ch = txt[j];
      if (ch === "\\" && txt[j + 1] === "n") { out += "\n"; j += 2; continue; }
      if (ch === "\\" && txt[j + 1] === '"') { out += '"'; j += 2; continue; }
      if (ch === "\\" && txt[j + 1] === "\\") { out += "\\"; j += 2; continue; }
      if (ch === '"' && txt[j - 1] !== "\\") break;
      if (ch !== "\\") out += ch;
      j++;
    }
    if (out.length > bestLen) { bestLen = out.length; best = out; }
    i = j;
  }
}
console.log("files", files.length, "bestLen", bestLen);
const p = "D:/server-official-site/web/css/_recovered-from-transcript.css";
if (bestLen > 500) { fs.writeFileSync(p, best); console.log("wrote", p); }
else { process.exit(1); }
